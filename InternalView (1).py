...
TRANSACTIONS_FILE = "Inventory Transactions 2025-2026.xlsx"
...
def load_transactions_data():
    tx = pd.read_excel(TRANSACTIONS_FILE)
    ...
    required_cols = [
        ...
        "Transaction Group Desc",
    ]
    missing = [c for c in required_cols if c not in tx.columns]
    if missing:
        raise ValueError(f"Transactions file is missing required columns: {missing}")

    tx = tx[tx["Transaction Group Desc"].isin(USAGE_TRANSACTION_TYPES)].copy()
    tx["Txn Inv Item ID"] = tx["Txn Inv Item ID"].astype(str).str.strip()
    tx["Total Quantity"] = pd.to_numeric(tx["Total Quantity"], errors="coerce").fillna(0)

    tx["Month"] = pd.to_datetime(
        tx["Transaction Date YYYY MM"].astype(str).str.replace(" ", "-", regex=False) + "-01",
        errors="coerce",
    )
    tx = tx.dropna(subset=["Month"])
...
def negbinom_cdf(k, r, p):
    ...
    for i in range(0, k + 1):
        total += negbinom_pmf(i, r, p)

    return min(max(total, 0.0), 1.0)

@st.cache_data(show_spinner=False)
def build_item_forecast_table():
    tx = load_transactions_data().copy()

    all_months = pd.date_range(tx["Month"].min(), tx["Month"].max(), freq="MS")

    monthly = (
        tx.groupby(["Txn Inv Item ID", "Month"], as_index=False)["Total Quantity"]
        .sum()
        .rename(columns={"Txn Inv Item ID": "Item", "Total Quantity": "Monthly_Qty"})
    )

    monthly["Item"] = monthly["Item"].astype(str).str.strip()

    all_items = monthly["Item"].dropna().unique()
    full_index = pd.MultiIndex.from_product([all_items, all_months], names=["Item", "Month"])

    monthly = (
        monthly.set_index(["Item", "Month"])
        .reindex(full_index, fill_value=0)
        .reset_index()
    )

    summary = (
        monthly.groupby("Item", as_index=False)
        .agg(
            Monthly_Demand_Mean=("Monthly_Qty", "mean"),
            Monthly_Demand_Var=("Monthly_Qty", "var"),
            Monthly_Demand_Max=("Monthly_Qty", "max"),
            Months_Observed=("Monthly_Qty", "size"),
            Months_With_Demand=("Monthly_Qty", lambda s: int((s > 0).sum())),
        )
    )

    summary["Monthly_Demand_Var"] = summary["Monthly_Demand_Var"].fillna(0)

    summary["Variability_Ratio"] = np.where(
        summary["Monthly_Demand_Mean"] > 0,
        summary["Monthly_Demand_Var"] / summary["Monthly_Demand_Mean"],
        np.nan,
    )

    summary["Demand_Variability"] = np.where(
        (summary["Monthly_Demand_Mean"] <= 0) | (summary["Monthly_Demand_Var"] <= summary["Monthly_Demand_Mean"]),
        "Low Variability",
        "High Variability",
    )

    summary["Distribution_Used"] = np.where(
        summary["Demand_Variability"] == "Low Variability",
        "Poisson",
        "Negative Binomial",
    )

    days_factor = FORECAST_HORIZON_DAYS / 30.4375
    summary["Forecast_Period_Mean"] = summary["Monthly_Demand_Mean"] * days_factor
    summary["Forecast_Period_Var"] = np.maximum(summary["Monthly_Demand_Var"] * days_factor, 0)

    summary["NB_r"] = np.where(
        (summary["Demand_Variability"] == "High Variability") &
        (summary["Forecast_Period_Var"] > summary["Forecast_Period_Mean"]) &
        (summary["Forecast_Period_Mean"] > 0),
        (summary["Forecast_Period_Mean"] ** 2) / (summary["Forecast_Period_Var"] - summary["Forecast_Period_Mean"]),
        np.nan,
    )

    summary["NB_p"] = np.where(
        summary["NB_r"].notna() & (summary["Forecast_Period_Mean"] > 0),
        summary["NB_r"] / (summary["NB_r"] + summary["Forecast_Period_Mean"]),
        np.nan,
    )

    return summary

def apply_forecast_metrics(inventory_df, col_map):
    out = inventory_df.copy()
    item_col = col_map.get("Item")

    default_cols = {
        "Monthly_Demand_Mean": 0.0,
        "Monthly_Demand_Var": 0.0,
        "Monthly_Demand_Max": 0.0,
        "Months_Observed": 0,
        "Months_With_Demand": 0,
        "Variability_Ratio": np.nan,
        "Demand_Variability": "Low Variability",
        "Distribution_Used": "Poisson",
        "Forecast_Period_Mean": 0.0,
        "Forecast_Period_Var": 0.0,
        "NB_r": np.nan,
        "NB_p": np.nan,
        "Forecast_Daily_Demand": 0.0,
        "Stockout_Probability_30D": 0.0,
        "Recommended_Target_30D": 0,
        "Recommended_Reorder_Qty": 0,
        "Forecast_Stock_Status": "Healthy Stock",
        "Forecast_Stock_Label": "Healthy Stock",
        "Forecast_Insight": "Healthy stock",
    }

    if not item_col or item_col not in out.columns:
        for c, v in default_cols.items():
            out[c] = v
        return out

    try:
        forecast_df = build_item_forecast_table().copy()
        out[item_col] = out[item_col].astype(str).str.strip()
        ...
        out["Forecast_Period_Mean"] = out["Forecast_Period_Mean"].fillna(0)
        out["Forecast_Period_Var"] = out["Forecast_Period_Var"].fillna(0)
        out["Forecast_Daily_Demand"] = out["Forecast_Period_Mean"] / FORECAST_HORIZON_DAYS

        def stockout_probability(row):
            qty = row.get("Qty_Avail_Num", np.nan)
            mu = row.get("Forecast_Period_Mean", np.nan)
            dist = row.get("Distribution_Used", "")
            ...
            if dist == "Poisson":
                return max(0.0, 1 - poisson_cdf(qty, mu))

            if dist == "Negative Binomial":
                r = row.get("NB_r", np.nan)
                p = row.get("NB_p", np.nan)

                if pd.notna(r) and pd.notna(p) and r > 0 and 0 < p < 1:
                    return max(0.0, 1 - negbinom_cdf(qty, r, p))

                return max(0.0, 1 - poisson_cdf(qty, mu))

            return max(0.0, 1 - poisson_cdf(qty, mu))

        out["Stockout_Probability_30D"] = out.apply(stockout_probability, axis=1)

        out["Recommended_Target_30D"] = np.ceil(out["Forecast_Period_Mean"]).fillna(0).astype(int)
        out["Recommended_Reorder_Qty"] = np.maximum(
            0,
            out["Recommended_Target_30D"] - out["Qty_Avail_Num"].fillna(0)
        ).astype(int)

        out["Forecast_Stock_Status"] = np.select(
            [
                out["Stockout_Probability_30D"] >= 0.60,
                out["Stockout_Probability_30D"] >= 0.25,
            ],
            [
                "Low / Risk of Stockout",
                "Medium",
            ],
            default="Healthy Stock"
        )

        out["Forecast_Stock_Label"] = np.where(
            out["Forecast_Period_Mean"] <= 0,
            out["Stock_Status"],
            out["Forecast_Stock_Status"]
        )

        out["Forecast_Insight"] = np.select(
            [
                out["Forecast_Period_Mean"] <= 0,
                out["Forecast_Stock_Label"] == "Low / Risk of Stockout",
                out["Forecast_Stock_Label"] == "Medium",
            ],
            [
                "No recent demand in transaction history",
                "Restock recommended",
                "Monitor soon",
            ],
            default="Healthy stock"
...
df = apply_forecast_metrics(df, col_map)
...
if show_low_stock_only:
    if user_role == "Inventory Planning View" and "Forecast_Stock_Label" in filtered_df.columns:
        filtered_df = filtered_df[filtered_df["Forecast_Stock_Label"] == "Low / Risk of Stockout"]
    else:
...
# =========================================================
if user_role == "Inventory Planning View" and "Forecast_Stock_Label" in filtered_df.columns:
    low_stock_count = int((filtered_df["Forecast_Stock_Label"] == "Low / Risk of Stockout").sum())
else:
...
if user_role == "Business Unit View":
    ...
    m1.metric("Items Shown", len(filtered_df))
    m2.metric("Forecasted Low Stock", low_stock_count)
...
def render_product_card(row, idx_prefix, user_role="Business Unit View"):
    ...
    base_status = row.get("Stock_Status", "Unknown")
    status = row.get("Forecast_Stock_Label", base_status) if user_role == "Inventory Planning View" else base_status
    ...
    st.markdown(
        ...
        <div class="product-card">
            ...
            <div class="product-sub"><b>Units on Hand:</b> {int(qty_on_hand) if pd.notna(qty_on_hand) else "N/A"} {uom_val}</div>
    ...
    if user_role == "Inventory Planning View":
        ...
        demand_variability = safe_string(row.get("Demand_Variability", ""))
        distribution_used = safe_string(row.get("Distribution_Used", ""))
        forecast_mean_30d = row.get("Forecast_Period_Mean", np.nan)
        stockout_prob_30d = row.get("Stockout_Probability_30D", np.nan)
        reorder_qty = row.get("Recommended_Reorder_Qty", np.nan)
        forecast_insight = safe_string(row.get("Forecast_Insight", ""))
        ...
        with st.expander("Forecasting details", expanded=False):
            st.markdown(
                f"""
                <div class="detail-box">
                    <div class="product-sub"><b>30-Day Forecast Demand:</b> {round(forecast_mean_30d, 2) if pd.notna(forecast_mean_30d) else 'N/A'}</div>
                    <div class="product-sub"><b>Demand Variability:</b> {demand_variability or 'N/A'}</div>
                    <div class="product-sub"><b>Distribution Used:</b> {distribution_used or 'N/A'}</div>
                    <div class="product-sub"><b>30-Day Stockout Probability:</b> {round(stockout_prob_30d * 100, 1) if pd.notna(stockout_prob_30d) else 'N/A'}%</div>
                    <div class="product-sub"><b>Forecast Stock Insight:</b> {forecast_insight or 'N/A'}</div>
                    <div class="product-sub"><b>Recommended Reorder Qty:</b> {int(reorder_qty) if pd.notna(reorder_qty) else 0}</div>
...
if is_internal_view:
    ...
    if user_role == "Inventory Planning View":
        forecast_view_df = filtered_df.copy()
        healthy_count = int((forecast_view_df["Forecast_Stock_Label"] == "Healthy Stock").sum()) if "Forecast_Stock_Label" in forecast_view_df.columns else 0
        medium_count = int((forecast_view_df["Forecast_Stock_Label"] == "Medium").sum()) if "Forecast_Stock_Label" in forecast_view_df.columns else 0
        low_count = int((forecast_view_df["Forecast_Stock_Label"] == "Low / Risk of Stockout").sum()) if "Forecast_Stock_Label" in forecast_view_df.columns else 0
        total_reorder_qty = int(forecast_view_df["Recommended_Reorder_Qty"].fillna(0).sum()) if "Recommended_Reorder_Qty" in forecast_view_df.columns else 0

        s1, s2, s3, s4 = st.columns(4)
        s1.metric("Healthy Stock", healthy_count)
        s2.metric("Medium Risk", medium_count)
        s3.metric("Low Stock / Risk", low_count)
        s4.metric("Suggested Reorder Units", total_reorder_qty)

    sort_options = {
        "Inventory Planning View": ["Forecast Risk", "Availability", "Avg Daily Usage", "Name", "Unit cost", "Days of Supply"],
    ...
    if internal_sort_option == "Forecast Risk" and "Stockout_Probability_30D" in internal_df.columns:
        internal_df = internal_df.sort_values(
            by=["Stockout_Probability_30D", "Recommended_Reorder_Qty"],
            ascending=[False, False],
            na_position="last"
        )
    elif internal_sort_option == "Availability":
